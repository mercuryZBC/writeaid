...
func GenerateJWT(id int64, email string) (string, error) {
	...
	err = util.GetRedisClient().Set(context.Background(), tokenString, email, TokenExpireDuration).Err()
	err = util.GetRedisClient().Set(context.Background(), email, tokenString, TokenExpireDuration).Err()
	if err != nil {
		return "", err
	}
...
func DeleteJWT(email string) error {
	jwtToken := util.GetRedisClient().Get(context.Background(), email).Val()
	err := util.GetRedisClient().Del(context.Background(), jwtToken).Err()
	if err != nil {
		return err
	}
	err = util.GetRedisClient().Del(context.Background(), email).Err()
	if err != nil {
		return err
	}
	return nil
...
func AuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		email, err := util.GetRedisClient().Get(context.Background(), tokenString).Result()
		if err == redis.Nil {
